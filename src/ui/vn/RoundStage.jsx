 ...
 * not JSON.
 *
 * WHAT IS ON SCREEN, AND WHY IT IS NOT HIDDEN. Both axes for everybody in the
 * room, plus the player's own values (Part I.2). v1 concealed them and handed
 * the player a labelled lever instead, which is the worst of both. `Read her` is
 * the only hidden state left.
 *
...
export default function RoundStage({
  ...
  return (
    <div className="stage-fill flex flex-col">
      <SceneHeader
      ...
      <div className="relative flex min-h-0 flex-1 items-end justify-center overflow-hidden">
        {focusCard ? (
          ...
          <p className="mb-8 font-mono text-[0.625rem] uppercase tracking-[0.18em] text-faint">
            {t('game.alone')}
          </p>
        )}
        {thought ? (
